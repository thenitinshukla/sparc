...
SPARC (Spherical Plasma Approximation for Radial Coulomb) simulates Coulomb explosions in spherical nanoplasmas.
...
\begin{lstlisting}[caption={Serial bubble sort},label={lst:serial_sort}]
...
\begin{lstlisting}[caption={Electric field calculation},label={lst:serial_field}]
...
\subsection{Distributed Prefix Sum}
...
for (int b = 0; b < NUM_BINS; b++) {
    ...
    if (cumsum >= target * (splitter_idx + 1)) {
        splitters[splitter_idx++] =
            r2_min + (b + 1) * bin_width;
    }
}
\end{lstlisting}
...
\begin{lstlisting}[caption={K-way merge},label={lst:kway}]
struct Node {
    double r2;
    int index;
    int chunk;
...
while (!pq.empty()) {
    ...
    merge_order[out++] = n.index;
    pos[n.chunk]++;
    if (pos[n.chunk] < recv_counts[n.chunk]) {
        int idx = recv_displs[n.chunk] + pos[n.chunk];
        pq.push({recv_r2[idx], idx, n.chunk});
    }
}
\end{lstlisting}
...
\hline
Splitter selection & O(SP) & O(B) \\
Sorting & O(N log N) & O(N log P) \\
Energy & O(N$^2$) & O(N) \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\linewidth]{strong_scaling_plot.png}
\caption{Strong scaling performance of the optimized implementation.}
\label{fig:scaling}
\end{figure}
...
\caption{Total time vs. No. particle}
\label{fig:scaling}
\end{figure}
...
We presented an optimized MPI implementation for SPARC Coulomb explosion simulations. Histogram-based splitter selection removes the root bottleneck, k-way merge reduces sorting complexity, and prefix-sum energy evaluation enables simulations with large particle counts.
