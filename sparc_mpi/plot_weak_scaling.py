...
Generates: Execution Time and Efficiency plots with dual y-axes
"""
...
def find_result_directories():
    """Find all weak scaling result directories."""
    pattern = os.path.join(RESULTS_BASE, "weak_scaling_*")
...
def select_directory(dirs):
    ...
    print("\nAvailable weak scaling results:")
...
def plot_weak_scaling(nodes, times, timestamp=""):
    """Generate weak scaling plot with dual y-axes."""
    ...
    t_baseline = times[0]
    efficiency = (t_baseline / times) * 100
    
    # Main plot: Dual y-axes
    fig, ax1 = plt.subplots(figsize=(12, 7))
    
    # Left y-axis: Execution Time
    color_time = '#2ca02c'  # Green
    ax1.set_xlabel('Nodes', fontsize=14, fontweight='bold')
    ax1.set_ylabel('Execution Time (s)', fontsize=14, fontweight='bold')
    
    # Plot execution time (solid line)
    line1 = ax1.plot(nodes, times, 'o-', linewidth=2.5, markersize=10, 
                     color=color_time, label='Actual Time')
    # Plot ideal time (horizontal line)
    ideal_time = np.ones_like(nodes) * t_baseline
    line2 = ax1.plot(nodes, ideal_time, 's--', linewidth=2.5, markersize=8,
                     color='black', alpha=0.7, label=f'Ideal Time ({t_baseline:.2f}s)')
    ...
    line4 = ax2.plot(nodes, ideal_efficiency, 's--', linewidth=2.5, markersize=8,
                     color='black', alpha=0.7, label='Ideal (100%)')
    ...
    plt.title(f'Weak Scaling Analysis{title_suffix}', fontsize=16, fontweight='bold', pad=20)
    ...
    axes[0].plot(nodes, times, 'o-', linewidth=2, markersize=8, label='Actual Time', color=color_time)
    axes[0].axhline(y=t_baseline, color='black', linestyle='--', linewidth=2, 
    ...
    axes[1].plot(nodes, efficiency, 'o-', linewidth=2, markersize=8, label='Weak Scaling Efficiency', color=color_efficiency)
    axes[1].axhline(y=100, color='black', linestyle='--', linewidth=2, alpha=0.7, label='Ideal (100%)')
    axes[1].set_xlabel('Number of Nodes', fontsize=12)
    axes[1].set_ylabel('Efficiency (%)', fontsize=12)
    axes[1].set_title('Efficiency vs Nodes', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    output_name_detailed = f'weak_scaling_{timestamp}_detailed.png' if timestamp else 'weak_scaling_detailed.png'
    ...
    for i in range(len(nodes)):
        print(f"{nodes[i]:<8} {times[i]:<12.3f} {efficiency[i]:<15.2f}")
    print("="*60)

def main():
    print("="*60)
    print("  Weak Scaling Results Plotter")
    print("="*60)
    
    dirs = find_result_directories()
    if not dirs:
        print("\nNo weak scaling results found!")
    ...
    if nodes is not None:
        plot_weak_scaling(nodes, times, timestamp)
        
        # Copy plots to result directory
        import shutil
        plot_file = os.path.join(result_dir, "weak_scaling_plot.png")
        plot_detailed_file = os.path.join(result_dir, "weak_scaling_detailed.png")
        
        src = os.path.join(PLOTS_DIR, f'weak_scaling_{timestamp}.png')
        src_detailed = os.path.join(PLOTS_DIR, f'weak_scaling_{timestamp}_detailed.png')
