...
def create_simple_animation():
    """Create a simple animation with all particle species"""
    # Get all time steps
    output_dir = "output"
    ...
    electron_scatter = ax.scatter([], [], s=0.5, c='cyan', alpha=0.6, label='Electrons')
    ion_scatter = ax.scatter([], [], s=1.0, c='magenta', alpha=0.6, label='Ions')
    proton_scatter = ax.scatter([], [], s=0.8, c='yellow', alpha=0.6, label='Protons')
    ...
    # Add title
    title = ax.set_title('SPARC Simulation', color='white', fontsize=14, pad=15)
    ...
    def animate(frame_idx):
        ...
        title.set_text(f'SPARC Simulation - Time Step {time_step}')
    ...
    # Save animation
    print("Saving animation as simple_sparc_animation.gif...")
    anim.save('simple_sparc_animation.gif', writer='pillow', fps=5)
